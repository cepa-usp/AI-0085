package  
{
	...
	public class Main extends MovieClip
	{
		...
		private var randomK:Array = new Array();//escolhe os valores
		...
		private function btChecka(event:MouseEvent):void {
			if(respUser != ""){
				if(respUser == labelEq[randomTrue].value){
					...
					labelEx[randomTrue].setTextFormat(correct2);
					labelEx[randomTrue].x = labelEx[randomTrue].x + 2;
		...
		private function geraEq() {
			for (var i:uint = 1; i <= numEquations; i++) {
				randomB[i] = Math.pow( -1, rand(1, 2));
				randomA[i] = valores[rand(0,4)]*(-1);
				randomK[i] = valores[rand(0,4)]*(Math.pow(-1,rand(1,2)));
				equation[i] = randomA[i] + "^(" + randomK[i] + "*x)";
			}
			if (!comparaEq()) {
		...
		private function comparaEq():Boolean {
			for (var i:uint = 1; i <= numEquations; i++) {
				for (var j:uint = i+1; j <= numEquations; j++) {
					if ((randomA[i] == randomA[j] && randomK[i] == randomK[j]) || (randomA[i] == 1/randomA[j] && randomK[i] == -randomK[j]) || (randomK[i] == 0 && randomK[j] == 0) || ((randomB[i] < 0 && randomA[i] == 1) && (randomB[j] < 0 && randomA[j] == 1)) || ((randomB[i] > 0 && randomA[i] == 1) && (randomB[j] > 0 && randomA[j] == 1))) {
						geraEq();
						return true;
					}
				}
